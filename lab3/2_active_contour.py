import numpy as np
import cv2
from skimage.morphology import disk
from skimage import segmentation as seg
from skimage.filters import gaussian, rank
import matplotlib.pyplot as plt
import matplotlib.lines as mlines

import sys
if sys.platform == "darwin":
    import matplotlib
    matplotlib.use("TkAgg")  # due to a bug on MacOS, just ignore it...


im = cv2.imread("violin-and-hand.jpg")
im_gray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)  # gray scale
im_l = cv2.cvtColor(im, cv2.COLOR_BGR2LAB)[:, :, 0]  # lightness channel
im_v = cv2.cvtColor(im, cv2.COLOR_BGR2HSV)[:, :, 2]  # value channel

# =============== pre-processing for geodesic active contour ================ #

# after checking, value channel has the highest contrast among the 3 above
im_cleaned = im_v.copy()
im_cleaned = rank.median(im_cleaned, footprint=disk(3))
im_cleaned = gaussian(im_cleaned, 3)


# ==================== method 1: geodesic active contour ==================== #

gim = seg.inverse_gaussian_gradient(im_cleaned)
init_ls = np.zeros_like(im_gray, dtype=np.int8)
init_ls[:, 20:-50] = 1

# selected sets of parameters
parameters_sets = [
    {"smoothing": 1, "balloon": -0.65, "threshold": 0.55},
    {"smoothing": 2, "balloon": -0.65, "threshold": 0.55},
    {"smoothing": 1, "balloon": -0.75, "threshold": 0.55},
    {"smoothing": 1, "balloon": -0.65, "threshold": 0.60}
]

evolutions = []
ls = []

for parameters in parameters_sets:
    evolutions.append([])
    ls.append(seg.morphological_geodesic_active_contour(
        gim,
        num_iter=500,
        init_level_set=init_ls,
        **parameters,
        iter_callback=lambda x: evolutions[-1].append(x)))

im_seg = im.copy()
im_seg[np.where(1 - ls[0])] = (0, 0, 0)
cv2.imwrite("violin-and-hand_active_contour.jpg", im_seg)

# ================================== plots ================================== #

plt.rcParams["figure.figsize"] = [9, 12]

row = len(parameters_sets) + 1
col = 2

plt.subplot(row, col, 1)
plt.imshow(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))
plt.title("Original image")
plt.axis("off")

plt.subplot(row, col, 2)
plt.imshow(im_cleaned, cmap="gray")
plt.title("Pre-proccessed (denoised and gaussian blurred)")
plt.axis("off")

def show_params(parameters):
    return ", ".join([f"{k}={v}" for k, v in parameters.items()])

for i in range(len(parameters_sets)):
    plt.subplot(row, col, 3+i*2)
    plt.imshow(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))
    plt.contour(ls[i], [0.5], colors='r')
    plt.title(show_params(parameters_sets[i]))
    plt.axis("off")

    plt.subplot(row, col, 4+i*2)
    plt.imshow(ls[i], cmap="gray", vmin=0, vmax=255)
    plt.title("<-- evolution")
    plt.axis("off")
    contour_labels = []
    for n, color in ((0, 'g'), (100, 'y'), (300, 'b'), (500, 'r')):
        plt.contour(evolutions[i][n], [0.5], colors=color)
        # Use empty line to represent this contour in the legend
        legend_line = mlines.Line2D([], [], color=color, label=f"Iteration {n}")
        contour_labels.append(legend_line)
    plt.legend(handles=contour_labels, loc="lower right", fontsize=6)

plt.tight_layout()
plt.show()


